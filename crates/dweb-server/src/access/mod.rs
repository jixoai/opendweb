...
//! - [`gate`]：AccessGate 验证链聚合（C0/L1/L1b/L2）与 registry 热重载
//!   看护、rendezvous Op（task 1.5/1.6，design §8.2/§8.4）、per-owner
//!   在线表与连接配额（task 3.2 前半）
...
//!   + 注册回执签名 + status 运行态投影，task 3.1，design §6.2）
//!
//! e2e 集成矩阵（task 1.9）在 tests/（黑盒二进制 + 真 iroh relay/客户端）。
